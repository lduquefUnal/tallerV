...
// Definición de handlers para GPIO (LEDs, Display, Encoder, etc.)
GPIO_Handler_t userLed = {0};    // Pin H1
GPIO_Handler_t userDis1 = {0};   // Pin A0
GPIO_Handler_t userDis2 = {0};   // Pin A1
GPIO_Handler_t userDis3 = {0};   // Pin A4
GPIO_Handler_t userDis4 = {0};   // Pin B0
GPIO_Handler_t userClk = {0};    // Pin A12
GPIO_Handler_t userData = {0};   // Pin C8
GPIO_Handler_t userSw = {0};     // Pin C9

// Segmentos para display de 7 segmentos
...
PWM_Handler_t pwmLed = {0};
PWM_Handler_t pwmFilter = {0};

// Enumeraciones para estados del display y colores del LED
enum {
    unidades = 1,
    decenas,
    centenas,
...
// Definición de la máquina de estados finita (FSM)
typedef enum {
    FSM_IDLE,
    FSM_UPDATE_DISPLAY,
    FSM_ENCODER_UP,
...
int main (void){
	...
	init_System();
	init_Pwm();
	displayNumber(8); // se encienden todos y se apagan para evitar led fantasma
	config_SysTick_ms(0);
	configMagic();
	while(1){
...
void init_Pwm(void){
	// configuración del pwm led
	pwmLed.ptrTIMx = TIM4;
	pwmLed.config.channel = PWM_CHANNEL_3;
	pwmLed.config.prescaler = 1600;
	pwmLed.config.periodo = 1000;
	pwmLed.config.dutyCycle = 800;
	pwm_Config(&pwmLed);
	pwm_Enable_Output(&pwmLed);
    // confiración del pwm del regulador RC
	pwmFilter.ptrTIMx = TIM4;
	pwmFilter.config.channel = PWM_CHANNEL_4;
	pwmFilter.config.prescaler = 1600;
	pwmFilter.config.periodo = 1000;
	pwmFilter.config.dutyCycle = 750;
	pwm_Config(&pwmFilter);
	pwm_Enable_Output(&pwmFilter);
}
void init_System(void){
	...
	// Configurar el pin B8 (Rojo)
	ledRed.pGPIOx                          = GPIOB;
	ledRed.pinConfig.GPIO_PinNumber        = PIN_8 ;
	ledRed.pinConfig.GPIO_PinMode          = GPIO_MODE_ALTFN;     // Modo alternativo
	...
	ledRed.pinConfig.GPIO_PinAltFunMode    = AF2;       // Modo alternativo para TIM4
	gpio_Config(&ledRed);
	//
	...
	//Cargamos la configuracion  en los registros que gobiernan 	el puerto.
	gpio_Config(&userSw);
	// se agrega  la configuración del pin para el PWM
	PWM_Pin.pGPIOx                           = GPIOB;
	PWM_Pin.pinConfig.GPIO_PinNumber         = PIN_9 ;
	PWM_Pin.pinConfig.GPIO_PinMode           = GPIO_MODE_ALTFN;
	PWM_Pin.pinConfig.GPIO_PinOutputType     = GPIO_OTYPE_PUSHPULL;
	PWM_Pin.pinConfig.GPIO_PinPuPdControl    = GPIO_PUPDR_NOTHING;
	PWM_Pin.pinConfig.GPIO_PinOutputSpeed    = GPIO_OSPEED_MEDIUM;
	PWM_Pin.pinConfig.GPIO_PinAltFunMode     = AF2;
	gpio_Config(&PWM_Pin);
...
void Timer4_Callback(void){
	 pwm_Update_DutyCycle(&pwmLed, 1000);
}
...
void callback_ExtInt9(void){
	...
	if (!debounceInProgress && (currentTime - lastDebounceTime) > DEBOUNCE_DELAY_MS) {
		...
		printf("Tiempo de ejecución: %llu ms\n", getTicks_ms());
	}
}
/*
 * está función se ejecuta cada vez que se recibe un nuevo carácter
 */
