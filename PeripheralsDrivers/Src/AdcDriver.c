...
//		ADC1->CR1 &= ~(ADC_CR1_RES); // limpiamos los bits
//		ADC1->CR1 |= (0b11 << ADC_CR1_RES_Pos);  ambos funciona
		break;
	}
	...
	if(adcConfig->channel <= ADC_CHANNEL_9){
		// (adcConfig->channel *3) desplaza 3n posiciones
		ADC1->SMPR2 &= ~(0b111 <<(adcConfig->channel *3));
		ADC1->SMPR2 |= (adcConfig->samplingPeriod << (adcConfig->channel *3));
	}
	else{
		ADC1->SMPR1 &= ~(0b111 <<(adcConfig->channel *3));
		ADC1->SMPR1 |= (adcConfig->samplingPeriod << (adcConfig->channel *3));
...
void ADC_IRQHandler(void){
	...
	if(ADC1->SR & ADC_SR_EOC){
		...
		adcRawData = ADC1-> DR;  //tomamos el dato del Data Register
